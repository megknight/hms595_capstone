...
split_run_id_M <- 54623 
split_run_id_F <- 54626 
decomp_step <- "step"
...
## read in microdata
microdata_reg <- fread(file.path(dir,paste0("FILEPATH",microdata_version,".csv")))
microdata_cens <- fread(file.path(dir,paste0("FILEPATH",microdata_version,".csv")))
...
microdata[is.na(variance)] ## make sure variance is no longer missing

## Recalculate variance where too small
...
## use microdata to impute age and sex disaggregated sample size for ILO tabulations,
...
# NEW ILO TABULATIONS -----------------------------------------------------
new <- fread(file.path(dir,"FILEPATH"))[!is.na(obs_value),list(ihme_loc_id = ref_area,country = ref_area.label,year_id = time,survey_id = paste0(source.label," - ",time),sex_id = ifelse(grepl("M",sex),1,ifelse(grepl("F",sex),2,3)),age = classif1.label,data = obs_value/100,note1 = note_classif.label,note2 = note_source.label)]
...
new <- unique(new,by=c("survey_id","age_start","age_end","sex_id")) ## some overlap of data due to different formatting of same age group, and some where data is just slightly different
new <- new[age_end >= 19 & age_start <= 65]
new_og <- copy(new)

## identify "gold_standard survey_ids", defined as surveys where data exists in every 5-year age group between 15 and 69 for both sexes separately
...
## drop all sex_id 3 data where comparative-age data for each sex separately already exists (since it's just duplicative)
...
new[data > 1, data := max_data] ## will probably end up outliering these points, but for now let them slide]
new[data < 0, data := min_data] ## will probably end up outliering these points, but for now let them slide]
...
for (this.age in seq(15,65,5)) {
  ...
  merge_min[,V2 := sum(sex_id),by=survey_id] ## cut out most-detailed sex_id 3 points when
...
## drop all sex_id 3 data where comparative-age data for both sexes separately already exists (turns out to be all of sex_id 3, so get to skip some steps)
...
old[data > 1, data := max_data] ## will probably end up outliering these points, but for now let them slide]
old[data < 0, data := min_data] ## will probably end up outliering these points, but for now let them slide]
...
if (do_split) {
  ...
  source("FILEPATH")
  source("FILEPATH")
  gpr_M <- model_load(split_run_id_M,"raked")
  gpr_F <- model_load(split_run_id_F,"raked")
  gpr <- rbind(gpr_M,gpr_F)
  ...
  ## in cases where split data surpasses 1, relax the sex-pattern
  sexsplit[split_data > max_data,maximum := max(split_data),by=.(survey_id,age_group_id)]
  sexsplit[,maximum := mean(maximum,na.rm=T),by=.(survey_id,age_group_id)]  
  sexsplit[!is.na(maximum),deviation := split_data - data]
  sexsplit[!is.na(maximum),max_deviation := max_data - data] 
  sexsplit[!is.na(maximum),dev_rescale := max_deviation/max(deviation),by=.(survey_id,age_group_id)]
  ...
  bothsplit[,maximum := mean(maximum,na.rm=T),by=.(survey_id,sex_id,age)]  
  bothsplit[!is.na(maximum),deviation := split_data - data]
  bothsplit[!is.na(maximum),max_deviation := max_data - data] 
  bothsplit[!is.na(maximum),dev_rescale := max_deviation/max(deviation),by=.(survey_id,sex_id,age)]
  ...
  split[,maximum := mean(maximum,na.rm=T),by=.(survey_id,sex_id,age)]  
  split[!is.na(maximum),deviation := split_data - data] 
  split[!is.na(maximum),max_deviation := max_data - data] 
  split[!is.na(maximum),dev_rescale := max_deviation/max(deviation),by=.(survey_id,sex_id,age)] 
  split[!is.na(dev_rescale),rescaled_split := data + (dev_rescale * deviation)] 
  ...
  ## calculate proportional sample size splits based on observations in microdata
...
## check that no surveys were dropped that shouldn't have been
new_og[year_id >= year_start & !survey_id %in% df_both$survey_id]
old_og[year_id >= year_start & !survey_id %in% df_both$survey_id]

# OUTLIERING --------------------------------------------------------------

df_both[ihme_loc_id == "CHN",c("ihme_loc_id","location_id") := .("CHN_44533",44533)]
df_both[grepl("issp",tolower(survey_name)) & grepl("_|IND",ihme_loc_id),is_outlier := 1]
...
# WRITE OUTPUTS -----------------------------------------------------------
if (do_split) {
  write.csv(df_both,file.path(dir,"FILEPATH"),row.names=F)
} else {
  write.csv(df_both,file.path(dir,"FILEPATH"),row.names=F)
  write.csv(new_split,file.path(dir,"FILEPATH"),row.names=F)
  write.csv(old_split,file.path(dir,"FILEPATH"),row.names=F)
}
...
if (generate_covs) {
  ...
  write.csv(custom_covs[,-c("cv_pop_perc"),with=F],file.path(dir,"FILEPATH"),row.names=F)
  write.csv(custom_covs,file.path(dir,"FILEPATH"),row.names=F)
}
